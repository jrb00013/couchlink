...
echo "==> elevating install into Program Files\\Couchlink\\Helper (approve UAC once)…"
set +e
...
if [[ "$ec" != "0" ]]; then
  ...
  echo "    Or run packaging/windows/build-helper-installer.ps1 and double-click the Setup exe." >&2
  exit "$ec"
fi

echo "==> verifying helper pipe…"
cp -f "$ROOT/scripts/windows/call-helper.ps1" \
  "/mnt/c/Users/$(cmd.exe /c "echo %USERNAME%" 2>/dev/null | tr -d '\r')/AppData/Local/couchlink-run/call-helper.ps1" 2>/dev/null || true
