...
/// Slice one chunk out of `raw` at `offset`, bounded by the chunk's own
/// Length (RFC 4960 §3.2). Several chunk parsers walk `buf.len()` instead
/// of `header.value_length()`, which is only valid when the chunk is alone
/// in the buffer. Bundled packets (legal except INIT / INIT ACK / SHUTDOWN
/// COMPLETE) then read into the next chunk and fail — typically
/// `ErrChunkTooShort` on a minimum-size FORWARD-TSN followed by SACK/DATA.
fn slice_chunk(raw: &Bytes, offset: usize) -> Result<Bytes> {
    ...
    let declared = u16::from_be_bytes([raw[offset + 2], raw[offset + 3]]) as usize;
    if declared < CHUNK_HEADER_SIZE {
...
impl Packet {
    pub(crate) fn unmarshal(raw: &Bytes) -> Result<Self> {
        ...
        loop {
            ...
            let chunk_raw = slice_chunk(raw, offset)?;
            let ct = ChunkType(chunk_raw[0]);
            let c: Box<dyn Chunk + Send + Sync> = match ct {
                CT_INIT => Box::new(ChunkInit::unmarshal(&chunk_raw)?),
                CT_INIT_ACK => Box::new(ChunkInit::unmarshal(&chunk_raw)?),
                CT_ABORT => Box::new(ChunkAbort::unmarshal(&chunk_raw)?),
                CT_COOKIE_ECHO => Box::new(ChunkCookieEcho::unmarshal(&chunk_raw)?),
                CT_COOKIE_ACK => Box::new(ChunkCookieAck::unmarshal(&chunk_raw)?),
                CT_HEARTBEAT => Box::new(ChunkHeartbeat::unmarshal(&chunk_raw)?),
                CT_PAYLOAD_DATA => Box::new(ChunkPayloadData::unmarshal(&chunk_raw)?),
                CT_SACK => Box::new(ChunkSelectiveAck::unmarshal(&chunk_raw)?),
                CT_RECONFIG => Box::new(ChunkReconfig::unmarshal(&chunk_raw)?),
                CT_FORWARD_TSN => Box::new(ChunkForwardTsn::unmarshal(&chunk_raw)?),
                CT_ERROR => Box::new(ChunkError::unmarshal(&chunk_raw)?),
                CT_SHUTDOWN => Box::new(ChunkShutdown::unmarshal(&chunk_raw)?),
                CT_SHUTDOWN_ACK => Box::new(ChunkShutdownAck::unmarshal(&chunk_raw)?),
                CT_SHUTDOWN_COMPLETE => Box::new(ChunkShutdownComplete::unmarshal(&chunk_raw)?),
                _ => Box::new(ChunkUnknown::unmarshal(&chunk_raw)?),
            };
...
mod test {
    ...
    #[test]
    fn test_forward_tsn_bundled_with_sack_parses() -> Result<()> {
        // Chrome PR-SCTP: FORWARD-TSN over unordered DATA has no stream/SSN
        // pairs (RFC 3758 §3.2) and is commonly bundled with a SACK. The
        // unpatched parser walked `buf.len()` past the 8-byte FORWARD-TSN
        // and returned ErrChunkTooShort, dropping pad + video with it.
        let pkt = Packet {
            source_port: 5000,
        ...
        assert_eq!(parsed.chunks.len(), 2, "both bundled chunks must survive");
        let fwd = parsed.chunks[0]
            .as_any()
            .downcast_ref::<ChunkForwardTsn>()
            .expect("first chunk is FORWARD-TSN");
        assert_eq!(fwd.new_cumulative_tsn, 42);
