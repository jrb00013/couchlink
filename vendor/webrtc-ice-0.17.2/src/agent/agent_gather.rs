        ...
        for network_type in network_types {
            ...
            for url in &urls {
                ...
                tokio::spawn(async move {
                    ...
                    // url.host has had its [] brackets stripped by Url::parse_url; an IPv6
                    // literal glued directly to ":port" is ambiguous (which colon is the
                    // port separator?) and fails to resolve. Bracket it back, same as
                    // Url's own Display impl does a few lines up in url/mod.rs.
                    let bracketed_host = if url.host.contains("::") {
                        format!("[{}]", url.host)
                    } else {
                        url.host.clone()
                    };
                    let host_port = format!("{bracketed_host}:{}", url.port);
    ...
    pub(crate) async fn gather_candidates_relay(
        ...
        for url in urls {
            ...
            tokio::spawn(async move {
                ...
                // Same unbracketed-IPv6 ambiguity as gather_candidates_srflx above.
                let bracketed_host = if url.host.contains("::") {
                    format!("[{}]", url.host)
                } else {
                    url.host.clone()
                };
                let turn_server_addr = format!("{bracketed_host}:{}", url.port);

                let (loc_conn, rel_addr, rel_port) =
                    if url.proto == ProtoType::Udp && url.scheme == SchemeType::Turn {
                        // This local socket's family drives the resolve_addr(is_ipv4, ...)
                        // call inside turn::client::Client::new() (via local_addr.is_ipv4()).
                        // A hardcoded IPv4 "0.0.0.0:0" bind here means an IPv6-only TURN
                        // server can never resolve, no matter what network_types allow —
                        // this function doesn't even loop over network types, unlike
                        // gather_candidates_srflx above. Bind the family the URL actually
                        // needs instead.
                        let bind_addr = if url.host.contains("::") {
                            SocketAddr::new(Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 0).into(), 0)
                        } else {
                            SocketAddr::new(Ipv4Addr::new(0, 0, 0, 0).into(), 0)
                        };
                        let loc_conn = match net2.bind(bind_addr).await {
                            Ok(c) => c,
                            Err(err) => {
                                log::warn!(
